...
async fn simulate_deploy_account() {
    ...
    let deployment = account_factory
        .deploy_v1(Felt::from_hex_unchecked(salt_hex))
        .max_fee(max_fee)
        .nonce(nonce)
        .prepared()
        .unwrap();
...
async fn simulate_with_max_fee_exceeding_account_balance_returns_error_if_fee_charge_is_not_skipped()
 {
    ...
    let declaration =
        account.declare_v3(Arc::new(sierra_artifact), casm_hash).gas(u64::MAX).gas_price(u128::MAX);
...
async fn simulate_invoke_declare_deploy_account_with_either_gas_or_gas_price_set_to_zero_or_both_will_revert_if_skip_fee_charge_is_not_set()
 {
    ...
    for (gas_units, gas_price) in [(0, 0), (0, 1e18 as u128), (1e18 as u64, 0)] {
        ...
        let invoke_transaction =
            BroadcastedTransaction::Invoke(BroadcastedInvokeTransaction::V3(invoke_transaction));
        ...
        let declare_transaction =
            BroadcastedTransaction::Declare(BroadcastedDeclareTransaction::V3(declare_transaction));
